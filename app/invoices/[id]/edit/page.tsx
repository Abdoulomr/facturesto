...
export default function EditInvoicePage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setItems(invoice.items.map((item: CartItem & { id?: string }) => ({
                ...
                <div className="space-y-2 mb-4 max-h-52 overflow-y-auto">
                  {items.map((item) => (
                    <div key={item.itemId} className="flex items-center gap-2 py-2 border-b border-stone-100 last:border-0">
                      ...
                      <button onClick={() => removeItem(item.itemId)} className="text-stone-300 hover:text-red-400 text-lg leading-none transition-colors ml-1">×</button>
                    </div>
                  ))}
                </div>
              )}
